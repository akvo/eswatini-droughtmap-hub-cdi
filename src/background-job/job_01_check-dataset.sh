...
check_and_download_LST_dataset() {
    ...
    check_and_create_download_log_flat  "${DOWNLOAD_LST_BASE_URL}" "${DOWNLOAD_LST_PATTERN}" "LST"
}

check_and_download_NDVI_dataset() {
    printf "\n===Check and download NDVI dataset===\n\n"
    check_and_create_download_log_flat  "${DOWNLOAD_NDVI_BASE_URL}" "${DOWNLOAD_NDVI_PATTERN}" "NDVI"
}
